...
export default function Pipeline() {
  ...
  return (
    <section
      id="pipeline"
      className="bg-bg-primary py-24 px-6 relative"
      ...
      <div className="max-w-6xl mx-auto relative z-10">
        ...
        >
          {PIPELINE_STEPS.map((step, i) => (
            <motion.div key={step.id} custom={i} variants={item}>
              {/* Step Number */}
              <div
              ...
              >
                ...
                <div
                  ...
                  } px-2.5 py-1 rounded border-l-2 border-amber inline-block`}
                >
                  {step.output}
                </div>
              </div>
        ...
        >
          <div className="bg-[#050709] border border-border-accent rounded px-6 py-4 font-mono text-sm text-cyan overflow-x-auto whitespace-nowrap border-l-4 border-l-amber">
            <span className="text-amber">$</span> python3 vulnscope.py --target 192.168.56.0/24
            --output report.pdf
          </div>
        </motion.div>
      </div>
